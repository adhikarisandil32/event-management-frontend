...
export default function NewEventForm({ event, showEditEventForm, setShowEditEventForm, setTriggerRerender }) {
  const {
    register,
    handleSubmit,
  ...
  const submitFn = async (formData) => {
    ...
    if (!responseJSON?.success) {
      ...
      return setShowEditEventForm(false)
    }
  }
      ...
      <div className="fixed py-8 left-0 right-0 bottom-0 h-[75vh] bg-yellow-300 rounded-t-3xl">
        ...
        <div className="container">
          <div className="text-right">
            <button className="p-2" onClick={() => setShowEditEventForm(false)}>
              ❌
            </button>
          </div>
